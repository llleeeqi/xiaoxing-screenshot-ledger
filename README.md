...
真实来源 App 优先作为 `channel` 传递；来源未知时使用 OCR 识别到的微信、支付宝、手机银行等渠道。淘宝、天猫、京东、拼多多、美团等应用会在备注中追加“消费平台：App 名称”，其他来源追加“来源：渠道”。小星的弹窗接口没有 `tag` 参数，但 `channel` 正是其“自动添加交易渠道标签”功能使用的字段。
